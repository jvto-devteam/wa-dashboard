...
export async function POST(req: NextRequest) {
  ...
  if (!number_key) {
    return NextResponse.json({ status: "1003", message: "Invalid Number Key" }, { status: 401 });
  }

  const found = await getClientByKeys(api_key, number_key);
  if (!found) {
    return NextResponse.json({ status: "1003", message: "Invalid API Key or Number Key" }, { status: 403 });
  }

  if (!phone_no || !message) {
    return NextResponse.json({ status: "1006", message: "Missing required fields: phone_no, message" }, { status: 400 });
  }
  ...
  try {
    if (client.status !== "connected") {
      ...
      if (!ok) return NextResponse.json({ status: "1004", message: "WhatsApp is not connected" }, { status: 400 });
    }
