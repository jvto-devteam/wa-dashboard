...
export async function POST(req: NextRequest) {
  ...
  const { api_key, number_key, group_id, message } = body;
  ...
  if (!group_id || !message) {
    return withCors(NextResponse.json({ status: "1006", message: "Missing required fields: group_id, message" }, { status: 400 }));
  }
